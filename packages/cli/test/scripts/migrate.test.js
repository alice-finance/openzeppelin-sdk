...
import { Proxy, Contracts, toSemanticVersion } from '@alice-finance/oz-upgrades';
...
contract('migrate-manifest-version script', function(accounts) {
  ...
  context('for published project', function() {
    ...
    beforeEach('initialize proxies manually', async function() {
      const { events: eventsV1Proxy } = await this.app.methods
        .create(this.packageName, 'ImplV1', EMPTY_INITIALIZATION_DATA)
        .send({ from: owner });
